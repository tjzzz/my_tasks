...
node_central_stat <- function(g, in_out){
  degree(g, mode = in_out, normalized = T)
  closeness(g, mode=in_out, normalized = T)
...
plot1<-function(g, xy, year){
  png(paste0(year, "g.png"), width = 1000, height = 1000)
  
  plot(g, layout = xy, vertex.size = 3, vertex.label = NA,
      edge.color = grey(0.3), edge.arrow.mode = "-")
  #plot(g, vertex.size = 3, vertex.label = NA,
  #    edge.color = grey(0.3), edge.arrow.mode = "-")
  dev.off()  
}

plot2<-function(g, org.com, xy, year){
  V(g)$sg = org.com$membership
  
  V(g)$color = rainbow(max(V(g)$sg))[V(g)$sg]
  png(paste0(year, "g2.png"), width = 1000, height = 1000)
  #par(mar = c(0, 0, 0, 0))
  #set.seed(14)
  plot(g, layout = xy, vertex.size = 3, vertex.color = V(g)$color, 
